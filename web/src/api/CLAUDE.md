...
- `client.ts` — 统一 fetch wrapper：管理员使用同源 HttpOnly Cookie，API Key 身份注入 Bearer、解包 `{code,data,msg}`、401 自动登出
...
- **身份分离**：管理员状态不保存 JWT，仅通过 HttpOnly Cookie 恢复；只有 `apiKey` session 可持久化并注入 Bearer。
