...
当完整 `model` 没有命中同名分组时，可使用 `渠道名称/上游模型名称` 只请求指定渠道一次，例如 `openai/gpt-4o`。按第一个 `/` 分隔，因此 `openai/vendor/model` 会把 `vendor/model` 作为实际上游模型名。

- 分组名称始终优先，即使名称中包含 `/`
- 渠道与模型名称严格区分大小写
- 仅允许渠道当前 `Model` / `CustomModel` 内存快照中已有的模型
- 不执行负载均衡、重试、故障转移、会话保持或分组首 Token 超时
- 仍遵守渠道启用状态、Key 可用性和熔断器
- 配置了 `SupportedModels` 的 API Key 不能使用该能力
- 直连模型不会出现在 `/v1/models`
